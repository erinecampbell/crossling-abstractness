...
# asl ----
asl_ratings_subset <- readRDS("norms/asl/asl_ratings_subset.rds")
...
czech_instrument_data <-czech_instrument_data_WS %>%
  left_join((czech_ratings_subset %>% distinct()), by = c("item_definition" = "Czech")) %>%
  as.data.frame() %>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition))
...
danish_instrument_data <- bind_rows(danish_instrument_data_WG, 
  ...
  mutate(produces = as.factor(produces)) %>%
  left_join((danish_ratings_subset %>% distinct()), by = c("item_definition" = "Danish")) %>%
  as.data.frame() %>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>% 
  distinct()
...
dutch_instrument_data <- dutch_instrument_data_no_ratings %>%
  left_join((dutch_ratings_subset %>% distinct()), by = c("item_definition" = "Dutch")) %>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame()
...
american_english_instrument_data <- american_english_instrument_data_no_ratings %>%
    left_join((english_ratings_subset %>% distinct()), by = c("item_definition" = "English (American)")) %>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
...
australian_english_instrument_data <- australian_english_instrument_data_WS %>%
  left_join((english_ratings_subset %>% distinct()), by = c("item_definition" = "English (Australian)"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
...
british_english_instrument_data <- british_english_instrument_data_no_ratings%>%
  left_join((english_ratings_subset %>% distinct()), by = c("item_definition" = "English (British)"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  distinct() %>%
  as.data.frame()
...
irish_english_instrument_data <- irish_english_instrument_data_WS%>%
  left_join((english_ratings_subset %>% distinct()), by = c("item_definition" = "English (Irish)"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  distinct() %>%
  as.data.frame()
...
estonian_instrument_data <-estonian_instrument_data_WS %>%
  mutate(produces = as.factor(produces))%>%
  as.data.frame() %>%
  left_join((estonian_ratings_subset %>% distinct()), by = c("item_definition" = "Estonian")) %>%
  distinct()
...
finnish_instrument_data <- bind_rows(finnish_instrument_data_WGProd,
  ...
  left_join((finnish_ratings_subset %>% distinct()), by = c("item_definition" = "Finnish"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
...
french_european_instrument_data <- bind_rows(french_european_instrument_data_WG, 
  ...
  left_join((french_ratings_subset %>% distinct()), by = c("item_definition" = "French (French)"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  distinct() %>%
  as.data.frame()
...
german_instrument_data <- german_instrument_data_WS%>%
  left_join((german_ratings_subset %>% distinct()), by = c("item_definition" = "German"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  distinct() %>%
  as.data.frame()
...
greek_instrument_data <- greek_instrument_data_WS%>%
  left_join((greek_ratings_subset %>% distinct()), by = c("item_definition" = "Greek (Cypriot)"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  distinct() %>%
  as.data.frame()
...
hebrew_instrument_data <- bind_rows(hebrew_instrument_data_WG, 
  ...
  left_join((hebrew_ratings_subset %>% distinct()), by = c("item_definition" = "Hebrew"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  distinct() %>%
  as.data.frame()
...
hungarian_instrument_data <- hungarian_instrument_data_WS %>%
  left_join((hungarian_ratings_subset %>% distinct()), by = c("item_definition" = "Hungarian"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  distinct() %>%
  as.data.frame()
...
irish_instrument_data <- get_instrument_data("Irish", form = "WS", administration_info = TRUE,item_info = TRUE) %>%
  drop_na(produces) %>%
  left_join((irish_ratings_subset %>% distinct()), by = c("item_definition" = "Irish"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  distinct() %>%
  as.data.frame()
...
japanese_instrument_data <- bind_rows(japanese_instrument_data_WG, 
  ...
  left_join((japanese_ratings_subset %>% distinct()), by = c("item_definition" = "Japanese"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  distinct() %>%
  as.data.frame()
...
italian_instrument_data <- bind_rows(italian_instrument_data_WG, 
  ...
  left_join((italian_ratings_subset %>% distinct()), by = c("item_definition" = "Italian"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  distinct() %>%
  as.data.frame()
...
kigiriama_instrument_data <- bind_rows(kigiriama_instrument_data_WG, 
  ...
  left_join((kigiriama_ratings_subset %>% distinct()), by = c("item_definition" = "Kigiriama"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
...
kiswahili_instrument_data <- bind_rows(kiswahili_instrument_data_WG, 
  ...
  left_join((kiswahili_ratings_subset %>% distinct()), by = c("item_definition" = "Kiswahili"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame()
...
korean_instrument_data <- bind_rows(korean_instrument_data_WG, 
  ...
  left_join((korean_ratings_subset %>% distinct()), by = c("item_definition" = "Korean"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
...
latvian_instrument_data <- bind_rows(latvian_instrument_data_WG, 
  ...
  left_join((latvian_ratings_subset %>% distinct()), by = c("item_definition" = "Latvian"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>% 
  distinct()
...
norwegian_instrument_data <- bind_rows(norwegian_instrument_data_WG, 
  ...
  as.data.frame() %>%
  left_join((norwegian_ratings_subset %>% distinct()), by = c("item_definition" = "Norwegian"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  distinct()
...
persian_instrument_data <- bind_rows(persian_instrument_data_WG, 
  ...
  as.data.frame() %>%
  left_join((persian_ratings_subset %>% distinct()), by = c("item_definition" = "Persian"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  distinct()
...
russian_instrument_data <- bind_rows(russian_instrument_data_WG, 
  ...
  left_join((russian_ratings_subset %>% distinct()), by = c("item_definition" = "Russian"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
...
slovak_instrument_data <- bind_rows(slovak_instrument_data_WG, 
  ...
  left_join((slovak_ratings_subset %>% distinct()), by = c("item_definition" = "Slovak"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
...
spanish_argentinian_instrument_data <- spanish_argentinian_instrument_data_WS%>%
  left_join((spanish_ratings_subset %>% distinct()), by = c("item_definition" = "Spanish (Argentinian)"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
...
spanish_chilean_instrument_data <- spanish_chilean_instrument_data_WG%>%
  left_join((spanish_ratings_subset %>% distinct()), by = c("item_definition" = "Spanish (Chilean)"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
...
spanish_european_instrument_data <- spanish_european_instrument_data_no_ratings %>%
  mutate(produces = as.factor(produces))%>%
  left_join((spanish_ratings_subset %>% distinct()), by = c("item_definition" = "Spanish (European)"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
...
spanish_mexican_instrument_data<- spanish_mexican_instrument_data_no_ratings%>%
  left_join((spanish_ratings_subset %>% distinct()), by = c("item_definition" = "Spanish (Mexican)"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
...
spanish_peruvian_instrument_data <- spanish_peruvian_instrument_data_no_ratings%>%
  left_join((spanish_ratings_subset %>% distinct()), by = c("item_definition" = "Spanish (Peruvian)"))%>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
...
swedish_instrument_data <- bind_rows(swedish_instrument_data_WG, 
  ...
  left_join((swedish_ratings_subset %>% distinct()), by = c("item_definition" = "Swedish")) %>%
  mutate(produces = as.factor(produces),
         word_length = str_length(item_definition)) %>%
  as.data.frame() %>%
  distinct()
